...
import {
    Cell, CellType, DIRECTIONS, GameState, Move, NEIGHBORS, ReverseInstruction,
    SerializedCoordinate, Trace
} from './types'
...
export abstract class PathfindingStrategy {
    ...
    protected exploreNeighbors() {
        ...
        for (const neighbor of NEIGHBORS) {
            const { type, level } = this._stacker.cell?.[neighbor] ?? { type: CellType.WALL, level: 0 }
