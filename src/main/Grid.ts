...
export class Grid {
    ...
    constructor() {
        this.gridMap.set(Coordinates.serialize([0, 0]), { type: CellType.EMPTY, level: 0 })
    }
    ...
    public onGoalFound(goal: Coordinate, goalLevel: number, topStair: Coordinate) {
        ...
        for (const [coordinateString, cell] of this.gridMap) {
            if (cell.type === CellType.BLOCK) this._closestBlocks.enqueue(Coordinates.deserialize(coordinateString))
        }
